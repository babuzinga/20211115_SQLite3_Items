...
if (!empty($_FILES["images"])) {
  ...
  foreach ($_FILES["images"]["error"] as $key => $error) {
    if ($error == UPLOAD_ERR_OK) {
      ...
      $product_uuid = $db->escapeString($_POST['product_uuid']);
      ...
      $db->exec("
        INSERT OR IGNORE INTO images (uuid, product_uuid, filename, dt) 
        VALUES ('{$uuid}', '{$product_uuid}', '{$filename_new}', CURRENT_TIMESTAMP)
      ");
